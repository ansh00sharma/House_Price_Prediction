...
def ArtificialNN_2(x_train, y_train, x_test, y_test,sc):
    # initializing ANN
    ...
    ann.add(tf.keras.layers.Dense(units=13, activation='elu'))

    ann.add(tf.keras.layers.Dense(units=1, activation='elu'))

    def percentage_difference(y_true, y_pred):
        return np.mean(abs(y_pred / y_true - 1) * 100)
